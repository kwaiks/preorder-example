...
class InvoicePrinter extends FPDF
{
    ...
    public function __construct($size = 'A4', $currency = 'Rp', $language = 'en')
    {
        $this->items              = [];
        $this->totals             = [];
        $this->addText            = [];
        $this->firstColumnWidth   = 150;
    ...
    public function addTotal($name, $value, $colored = false)
    {
        ...
        $t['value'] = $value;
        if (is_numeric($value)) {
            $t['value'] = $this->currency . ' ' . number_format($value, 0, $this->referenceformat[0],
    ...
    public function Header()
    {
        ...
        if (!isset($this->productsEnded)) {
            $width_other = ($this->document['w'] - $this->margins['l'] - $this->margins['r'] - $this->firstColumnWidth - 1);
            $this->SetTextColor(50, 50, 50);
            $this->Ln(12);
            $this->SetFont($this->font, 'B', 9);
    ...
    {
        $width_other = ($this->document['w'] - $this->margins['l'] - $this->margins['r'] - $this->firstColumnWidth - 2);
        $cellHeight  = 8;
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                ...
                $this->Cell($width_other, $cHeight, iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_B,
                    $this->currency . ' ' . number_format($item['price'], 0, $this->referenceformat[0],
        ...
        if ($this->totals) {
            foreach ($this->totals as $total) {
                ...
                $this->SetFont($this->font, 'b', 8);
                $this->Cell(1+$this->firstColumnWidth, $cellHeight, iconv(self::ICONV_CHARSET_INPUT, self::ICONV_CHARSET_OUTPUT_B, $total['name']), 0, 0, 'L',
                    1);
                $this->Cell($this->columnSpacing, $cellHeight, '', 0, 0, 'L', 0);
                $this->SetFont($this->font, 'b', 8);
                $this->SetFillColor($bgcolor, $bgcolor, $bgcolor);
